...
def create_customer_drive_folder(doc, method):
    ...
    """
    try:
        # Ensure the 'Customer' folder exists under 'Administrator's Drive'
        admin_drive_folder = frappe.db.get_value("Drive Entity", {"title": "Administrator's Drive", "is_group": 1})
        if not admin_drive_folder:
            frappe.throw("Administrator's Drive folder does not exist. Please create it manually.")

        customer_folder = ensure_folder_exists("Customer", admin_drive_folder)
